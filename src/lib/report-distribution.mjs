...
// One message through notifications/email.mjs's batch sendEmail (Slice 2C:
// `{ messages }` in, one `{ to, code, outcome, providerMessageId }` per
// message out). A missing result (empty recipient) is treated as a retryable
// server error so the leg is retried with backoff rather than dropped.
async function sendOneEmail(message, adapters) {
  const options = adapters?.email ? { adapter: adapters.email } : {};
  const results = await sendEmail({ messages: [message] }, options);
  return results[0] ?? { outcome: "retryable", code: "server_error", providerMessageId: null };
}
...
async function runImmediateEmailSends({ client, legs, adapters, appUrl, now, maxAttempts }) {
  ...
  for (const leg of sendable) {
    ...
    const result = await sendOneEmail({ to: leg.recipientEmail, subject, text }, adapters);
    await applyOutcome({
      client,
      leg,
...
async function runDigestSends({ client, legs, adapters, appUrl, now, maxAttempts }) {
  ...
  for (const groupLegs of groups.values()) {
    ...
    const result = await sendOneEmail({ to: groupLegs[0].recipientEmail, subject, text }, adapters);
