...
export function registerBillingRoutes(router, { authenticate, sendJson, readBody }) {
  ...
  async function guardRuleWrite(auth, scopeType, scopeId, organizationId, response) {
    if (scopeType === "organization") {
      const guard = await requireAuthOrgAdminRow(auth, organizationId);
      if (!guard.allowed) {
  ...
  router.register("POST", "/org/:orgId/feature-flag-rules", (request, response, { env, params }) =>
    withAuth(request, response, env, async (auth) => {
      ...
      if (!(await guardRuleWrite(auth, body.payload.scopeType, body.payload.scopeId, params.orgId, response))) return;
  ...
  router.register("PATCH", "/org/:orgId/feature-flag-rules/:id", (request, response, { env, params }) =>
    withAuth(request, response, env, async (auth) => {
      ...
      if (!(await guardRuleWrite(auth, existing.scope_type, existing.scope_id, guardOrgId, response))) return;
