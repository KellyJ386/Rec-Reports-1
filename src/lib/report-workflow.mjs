...
// Exported for DR-19's event_type generation and for tests. Mirrors
// internal.enqueue_report_workflow's own algorithm (0060's create-or-replace
// of the 0053 RPC): an action carrying its own non-empty `eventType` (WO-21's
// per-defect create_work_order actions) uses it verbatim; every other action
// falls back to the original `${type}:${index}` composition, byte-identical
// to this function's pre-WO-21 behavior.
