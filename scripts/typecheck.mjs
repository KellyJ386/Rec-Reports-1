...
// already covered by another code's policy; reports.workflow.manage is
// reserved for DR-18/DR-20 with no route or policy yet.
// reports.distribution.manage graduated out of this set in 0054
// (DR-21): it now gates report_distribution_lists' INSERT/UPDATE/DELETE
// policy directly (see that migration's has_permission(...,
// 'reports.distribution.manage') literal), so it is picked up by the
// anyMigrationCodes scan below like every other RLS-wired code. Any other
// code that stops appearing in a migration (or a new code that's added
// without one) is a real regression of the kind S-5 itself fixed for
